...
export default function Products() {
  ...
  // Sync activeCategory chip with URL param
  useEffect(() => {
  ...
  return (
    <>
      <style>{`
        ...
        .kb-p-unit { font-size: 11px; color: var(--kb-muted); margin-bottom: 8px; }
          ...
          {loading ? (
            ...
            <div className="kb-products-grid">
              {sorted.map((p) => {
                ...
                return (
                  <div key={p.id} className="kb-p-card">
                    {/* Badge */}
                    {p.badge && (
                    ...
                    <div className="kb-p-img-wrap">
                      {p.imageUrl || p.image || p.img
                        ? <img src={p.imageUrl || p.image || p.img} alt={p.name}
                    ...
                    {p.inStock === false ? (
                      ...
                      <button className="kb-p-add-btn" onClick={() => addToCart(p)}>
                        + Add
                      </button>
                    ) : (
                      <div className="kb-p-stepper">
                        ...
                        <button className="kb-p-step-btn" onClick={() => increaseQty(p.id)}>+</button>
                      </div>
                    )}
